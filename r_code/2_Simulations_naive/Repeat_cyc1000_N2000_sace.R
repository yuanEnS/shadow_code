...
# 2. estimation -----------------------------------------------------------
  ...
  dat_exist <- dat[dat$S==1,]
  dat_exist <- dat_exist[dat_exist$R==1,]
  dat_exist_T <- dat_exist[dat_exist$Z==1,]
  dat_exist_C <- dat_exist[dat_exist$Z==0,]
  sace_value <- mean(dat_exist_T$Y) - mean(dat_exist_C$Y)

  var_sace <- var(dat_exist_T$Y)/nrow(dat_exist_T) + var(dat_exist_C$Y)/nrow(dat_exist_C)
  lower_interval <- sace_value - 1.96*sqrt(var_sace)
  upper_interval <- sace_value +  1.96*sqrt(var_sace)
  coverage <- 1
  if(lower_interval > true_causal | upper_interval < true_causal){
    coverage <- 0
  }
# 5. return results -------------------------------------------------------
...
print(time)
save.image(paste("Repeat",cyc,"_N",2000,".RData",sep = ''))
