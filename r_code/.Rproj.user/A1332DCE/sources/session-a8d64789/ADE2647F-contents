...
ELL <- function(seeds){
  ...
  # 4. Estimation for the coverage ------------------------------------------

  
  CEs_bootstrap <- c()
  
  for(i in 1:recycle_times){
    seed_yuan = seeds*recycle_times + i - 1
    set.seed(seed_yuan)
...
res <- parLapply(cl,(cyc*sub_procedure + 1):(cyc*(sub_procedure+1)),ELL)
CEs <- NULL
for(i in 1:cyc){
...
print(time)
save.image(paste("Repeat",cyc,"_N",5000,"_",sub_procedure,".RData",sep = ''))
